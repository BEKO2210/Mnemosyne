#!/usr/bin/env python3
"""
onboarding.py — Mnemosyne first-run setup.

Asks the user:
  1. How they're using Mnemosyne (work / personal / combo)
...
Seeds the entity_registry with confirmed data so Mnemosyne knows your world
from minute one — before a single session is indexed.

Usage:
    python3 -m mnemosyne.onboarding
    or: mnemosyne init
"""

from pathlib import Path
from mnemosyne.entity_registry import EntityRegistry
from mnemosyne.entity_detector import detect_entities, scan_for_detection
...
def _ask_mode() -> str:
    _header("Welcome to Mnemosyne")
    print("""
  Mnemosyne is a personal memory system. To work well, it needs to know
...
""")
    print("  How are you using Mnemosyne?")
    print()
...
def _ask_projects(mode: str) -> list:
    ...
    _hr()
    print("""
  What are your main projects? (These help Mnemosyne distinguish project
...
def _warn_ambiguous(people: list) -> list:
    ...
    """
    from mnemosyne.entity_registry import COMMON_ENGLISH_WORDS
...
def _generate_aaak_bootstrap(
    ...
    """
    mnemosyne_dir = Path(config_dir) if config_dir else Path.home() / ".mnemosyne"
    ...
    registry_lines = [
        "# AAAK Entity Registry",
        "# Auto-generated by mnemosyne init. Update as needed.",
        "",
        "## People",
    ]
    for p in people:
    ...
    (mnemosyne_dir / "aaak_entities.md").write_text("\n".join(registry_lines))
    ...
    (mnemosyne_dir / "critical_facts.md").write_text("\n".join(facts_lines))
...
def run_onboarding(
  ...
  Mnemosyne will check the context before treating them as person names.
...
""")
    ...
    print("\n  AAAK entity registry: ~/.mnemosyne/aaak_entities.md")
    print("  Critical facts bootstrap: ~/.mnemosyne/critical_facts.md")
